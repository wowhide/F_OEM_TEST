...
//PDF固定文
define('PDF_FIXED_TITLE', "故 %s 様アプリ案内書");
...
define('PDF_FIXED_SENTENCE_06', "お名前・命日・写真・法要スケジュール");
define('PDF_FIXED_SENTENCE_07', "などの情報が既に入っています。");
...
class comGuidanceIssue
{
    ...
    public static function issue(array $deceasedInfo, $fileDir) {
        ...
        $pdf->pages[0]->drawText(
                sprintf(PDF_FIXED_TITLE, $deceasedInfo['deceased_name']), 16, 790, 'UTF-8');

        //葬家様名
        $pdf->pages[0]->setFont($font, 14);
        $pdf->pages[0]->drawText(
                sprintf(PDF_FIXED_SENTENCE_01, $deceasedInfo['souke']), 32, 700, 'UTF-8');
        //あいさつ文
        $pdf->pages[0]->setFont($font, 12);
        // $pdf->pages[0]->drawText(
        //         sprintf(PDF_FIXED_SENTENCE_02, $deceasedInfo['souke']), 39, 707, 'UTF-8');
        $pdf->pages[0]->drawText(
                sprintf(PDF_FIXED_SENTENCE_03, $deceasedInfo['deceased_name']), 74, 564, 'UTF-8');
        $pdf->pages[0]->drawText(
                sprintf(PDF_FIXED_SENTENCE_06), 74, 546, 'UTF-8');
        $pdf->pages[0]->drawText(
                sprintf(PDF_FIXED_SENTENCE_07), 74, 528, 'UTF-8');
        //故人様名
        // $pdf->pages[0]->drawText(
        ...
        $pdf->pages[0]->drawText(
                ...
                $deceasedInfo['deceased_name']), 315, 280, 'UTF-8');

        //QRコードを描画する
        $image = Zend_Pdf_Image::imageWithPath($fileDir . '/qr.jpg');
        $pdf->pages[0]->drawImage($image, 500, 250, 570, 325);
